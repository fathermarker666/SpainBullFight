...
public class PlayerStats : MonoBehaviour
{
    ...
    {
        Debug.Log("<color=orange>™±Æa®¸®Ï∂ÀÆ`</color>");

        if (IsDead) { Debug.Log("™±Æa§w¶∫§`°A©ø≤§∂ÀÆ`"); return; }

        if (IsInvulnerable)
        {
            Debug.Log($"™±Æa®¸∂À•¢±—°A•ÿ´eµLºƒ§§ (≥—ælÆ…∂°: {invulnerabilityTimer})");
            return;
        }
        ...
        Debug.Log($"<color=red>™±Æa¶©¶Â</color> HP ≥—§U: {currentHealth}");
