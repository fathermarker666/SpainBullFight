...
public class WeaponSwitcher : MonoBehaviour
{
    ...
    private GameObject cachedOriginalGun;
    ...
    private void Update()
    {
        if (gameFlow == null || handAnimatorController == null)
            ResolveReferencesIfNeeded();

        if (originalGun != null)
        {
            if (!originalGunScaleCached || cachedOriginalGun != originalGun)
                CacheOriginalGunScale();
    ...
    private void CacheOriginalGunScale()
    {
        if (originalGun == null)
        {
            cachedOriginalGun = null;
            originalGunScaleCached = false;
            return;
        }

        cachedOriginalGun = originalGun;
    ...
    private void ResolveReferencesIfNeeded()
    {
        ...
        if (handAnimatorController == null)
            handAnimatorController = BullfightSceneCache.GetLocalOrScene<BullfightHandAnimatorController>(this);
    }
}
